...
**RULE**：定义访问权限`<level>`必须为`0`或`1`。级别`1`字段的权限继承了级别`0`字段的权限。权限为`NONE`、`READ`和`WRITE`，`WRITE`也继承了`READ`权限。标准EPICS记录类型的所有字段除`VAL`、`CMD`（命令）和`RES`（重置）外都设置为1级。可选参数指定是否应捕获写入。请参阅下面关于捕获通道访问的一节介绍了如何使用此功能。如果未给定，则默认为`NOTRAPWRITE`。
